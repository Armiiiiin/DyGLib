"""
Save prediction feature added for challenging scene evaluation
...
def measure_inference_time(encoder, decoder, data, traj_loader, args, 
                           needs_edge_ids=False, initial_memory_backup=None,
                           n_warmup=10, n_runs=100):
    """
    
    Measures the full pipeline: encoder + decoder.
    For memory models, resets state before each run to ensure consistency.
    """
    encoder.eval()
    decoder.eval()
    
    B = min(args.batch_size, len(data.src_node_ids))
    src   = data.src_node_ids[:B]
    dst   = data.dst_node_ids[:B]
    times = data.node_interact_times[:B]
    edge_ids = data.edge_ids[:B]
    
    traj_batch = traj_loader.get_batch(src, times)
    valid = traj_batch['valid_mask']
    
    if valid.sum() == 0:
        print("[WARNING] No valid samples for benchmark, timing may be inaccurate")
        current = torch.zeros(B, 5, device=args.device)
        history = torch.zeros(B, traj_loader.hist_horizon, 4, device=args.device)
        src_bench = src
        dst_bench = dst
        times_bench = times
        edge_ids_bench = edge_ids
    else:
        valid_np = valid.numpy()
        src_bench      = src[valid_np]
        dst_bench      = dst[valid_np]
        times_bench    = times[valid_np]
        edge_ids_bench = edge_ids[valid_np]
        current = traj_batch['current_state'][valid].to(args.device)
        history = traj_batch['history_traj'][valid].to(args.device)
    
    # > memory model: stateful — reset before every run for reproducibility
    def _reset_memory():
    ...
    def _run_once():
        if needs_edge_ids:
            src_emb, _ = encoder.compute_src_dst_node_temporal_embeddings(
                src_bench, dst_bench, times_bench, edge_ids_bench
            )
        else:
            src_emb, _ = encoder.compute_src_dst_node_temporal_embeddings(
                src_bench, dst_bench, times_bench
            )
        _ = decoder(src_emb, current, history_traj=history)
    
    with torch.no_grad():
        # > warmup: fill CUDA caches, JIT kernels
    ...
    return {
        ...
        'per_sample_ms': float(times_ms.mean() / actual_B),
        'fps':           float((actual_B * 1000.0) / times_ms.mean()),
...
def count_parameters(encoder, decoder, lane_encoder=None, map_fusion=None):
    def count(model):
        return sum(p.numel() for p in model.parameters() if p.requires_grad)
    
    enc_params = count(encoder)
    dec_params = count(decoder)
    ...
    if lane_encoder:
        lp = count(lane_encoder)
        total += lp
        print(f"  LaneEncoder: {lp:>10,}")
    if map_fusion:
        mp = count(map_fusion)
        total += mp
        print(f"  MapFusion:   {mp:>10,}")
...
class GRUHistoryEncoder(nn.Module):
    def __init__(self, hist_horizon=50, input_dim=4, embed_dim=128,
                 num_layers=2, dropout=0.1):
        super().__init__()
        self.input_proj = nn.Linear(input_dim, embed_dim)
        self.gru = nn.GRU(embed_dim, embed_dim, num_layers=num_layers,
                          batch_first=True,
                          dropout=dropout if num_layers > 1 else 0.0)
        self.norm = nn.LayerNorm(embed_dim)

    def forward(self, hist_traj, heading=None):
        B, T, _ = hist_traj.shape
        if heading is not None:
            cos_h = torch.cos(heading).view(B, 1, 1)
            sin_h = torch.sin(heading).view(B, 1, 1)
            rot_x  = cos_h * hist_traj[..., 0:1] + sin_h * hist_traj[..., 1:2]
            rot_y  = -sin_h * hist_traj[..., 0:1] + cos_h * hist_traj[..., 1:2]
            rot_vx = cos_h * hist_traj[..., 2:3] + sin_h * hist_traj[..., 3:4]
            rot_vy = -sin_h * hist_traj[..., 2:3] + cos_h * hist_traj[..., 3:4]
            hist_traj = torch.cat([rot_x, rot_y, rot_vx, rot_vy], dim=-1)
        x = self.input_proj(hist_traj)
        out, _ = self.gru(x)
...
class MultiModalMLPDecoder(nn.Module):
    ...
    def __init__(self, embed_dim: int, pred_horizon: int = 50, 
        ...
        if use_history:
            ...
            elif hist_encoder_type == 'gru':
                self.hist_encoder = GRUHistoryEncoder(
                    hist_horizon=hist_horizon,
                    input_dim=4,
                    embed_dim=hidden_dim,
                    num_layers=2,
            ...
            input_dim = embed_dim + 5 + hidden_dim
        else:
    ...
    def forward(self, node_embed, current_state=None, history_traj=None, **kwargs):
        ...
        if self.use_history and history_traj is not None:
            if self.hist_encoder_type in ['transformer', 'gru']:
...
class KinematicLoss(nn.Module):
    """Penalize physically impossible motions (acceleration + steering rate)."""
    
    def __init__(self, max_accel=4.0, max_steering_rate=0.52, dt=0.1):
        super().__init__()
        self.max_accel = max_accel
        self.max_steering_rate = max_steering_rate
        self.dt = dt

    def forward(self, pred_traj, current_state=None):
        """pred_traj: [B, T, 2]"""
        # Velocity from positions
        velocities = (pred_traj[:, 1:, :] - pred_traj[:, :-1, :]) / self.dt  

        
        accelerations = (velocities[:, 1:, :] - velocities[:, :-1, :]) / self.dt  
        accel_mag = torch.norm(accelerations, dim=-1)                              
        accel_loss = (F.relu(accel_mag - self.max_accel) ** 2).mean()
        ...
        heading_rate = heading_change.abs() / self.dt                             
        steering_loss = (F.relu(heading_rate - self.max_steering_rate) ** 2).mean()

       
        speed = torch.norm(velocities, dim=-1)                                    
        smoothness_loss = ((speed[:, 1:] - speed[:, :-1]) ** 2).mean()

        return 1.0 * accel_loss + 1.0 * steering_loss + 0.1 * smoothness_loss
...
class SplitDataLoader:
    ...
    def _load_split(self, split: str):
        # Check if batched files exist
        batch0_csv = self.data_dir / f'{split}_batch0.csv'
        regular_csv = self.data_dir / f'{split}.csv'
        
        if batch0_csv.exists():
            # Load batched files
            print(f"  Loading batched {split} data...")
            return self._load_batched_split(split)
        elif regular_csv.exists():
            # Load regular files
            return self._load_regular_split(split)
        else:
            raise FileNotFoundError(f"No data found for split '{split}'")

    def _load_regular_split(self, split: str):
        """Load from regular single files"""
    ...
    def _load_batched_split(self, split: str):
        """Load from batched files with node ID remapping"""
        batch_files = sorted(self.data_dir.glob(f'{split}_batch*.csv'))
        num_batches = len(batch_files)
        
        print(f"    Found {num_batches} batches")
        
        # Track global node offset
        ...
        for i in range(num_batches):
            # Load CSV
            batch_df = pd.read_csv(self.data_dir / f'{split}_batch{i}.csv')
            
            # Find max node ID in this batch
            max_node_in_batch = max(batch_df['u'].max(), batch_df['i'].max())
            
            # Remap node IDs in CSV
            batch_df['u'] += global_node_offset
            batch_df['i'] += global_node_offset
            all_dfs.append(batch_df)
            
            # Load and remap trajectories
            with open(self.data_dir / f'{split}_batch{i}_trajectory.pkl', 'rb') as f:
                batch_traj = pickle.load(f)
            
            for old_key, value in batch_traj['future_traj'].items():
                node_id, ts = old_key
                new_key = (node_id + global_node_offset, ts)
                traj_data['future_traj'][new_key] = value
            
            for old_key, value in batch_traj['current_state'].items():
                node_id, ts = old_key
                new_key = (node_id + global_node_offset, ts)
                traj_data['current_state'][new_key] = value
            
            for old_key, value in batch_traj.get('history_traj', {}).items():
                node_id, ts = old_key
                new_key = (node_id + global_node_offset, ts)
                traj_data['history_traj'][new_key] = value
            
            for old_key, value in batch_traj.get('is_target', {}).items():
                node_id, ts = old_key
                new_key = (node_id + global_node_offset, ts)
                traj_data['is_target'][new_key] = value
            
            traj_data['pred_horizon'] = batch_traj['pred_horizon']
            traj_data['hist_horizon'] = batch_traj.get('hist_horizon', 50)
            
            # Load and remap lanes
            lane_path = self.data_dir / f'{split}_batch{i}_lanes.pkl'
            if lane_path.exists():
                with open(lane_path, 'rb') as f:
                    batch_lane = pickle.load(f)
                
                for old_key, value in batch_lane['lanes'].items():
                    node_id, ts = old_key
                    new_key = (node_id + global_node_offset, ts)
                    lane_data['lanes'][new_key] = value
                
                lane_data['max_lanes'] = max(lane_data['max_lanes'], batch_lane['max_lanes'])
                lane_data['points_per_lane'] = max(lane_data['points_per_lane'], batch_lane['points_per_lane'])
                lane_data['style'] = batch_lane.get('style', 'v2xg-alg')
            
            # Load arrays
            all_edge_arrays.append(np.load(self.data_dir / f'{split}_batch{i}.npy'))
            all_node_arrays.append(np.load(self.data_dir / f'{split}_batch{i}_node.npy'))
            
            # Update offset for next batch
            global_node_offset += (max_node_in_batch + 1)
            
            if (i + 1) % 5 == 0 or i == num_batches - 1:
                print(f"    Processed batch {i+1}/{num_batches}, {len(traj_data['future_traj'])} total trajs")
        
        # Concatenate everything
        df = pd.concat(all_dfs, ignore_index=True)
        edge_feats = np.vstack(all_edge_arrays)
        node_feats = np.vstack(all_node_arrays)
        
        if not lane_data['lanes']:
            lane_data = None
        
        print(f"    Final: {len(traj_data['future_traj'])} trajectories")
...
def create_filtered_data_loader(data, traj_loader, batch_size, shuffle=True):
    """Create loader with only edges that have trajectory data"""
    from tqdm import tqdm
    
    valid_indices = []
    
    print("Filtering to prediction edges...")
    for idx in tqdm(range(len(data.src_node_ids)), desc="  Scanning edges"):
        src = data.src_node_ids[idx]
        ts = data.node_interact_times[idx]
        key = (int(src), float(ts))
        
        if key in traj_loader.future_traj:
            valid_indices.append(idx)
    
    print(f"  Prediction edges: {len(valid_indices):,}/{len(data.src_node_ids):,}")
    
    return get_idx_data_loader(valid_indices, batch_size, shuffle=shuffle)
...
def get_args():
    ...
    parser.add_argument('--model_name', type=str, default='DyGFormer',
                       choices=['DyGFormer', 'TGAT', 'TGN', 'JODIE', 'DyRep', 'CAWN', 'TCL', 'GraphMixer'])
    parser.add_argument('--kinematic_weight', type=float, default=0.0,
                   help='Weight for kinematic loss (0=disabled)')
    ...
    parser.add_argument('--save_predictions', action='store_true',
                       help='Save per-sample test predictions for challenging case analysis')
...
def main():
    ...
    train_loader = create_filtered_data_loader(
        train_data, train_traj_loader, args.batch_size, shuffle=True
    )
    val_loader = create_filtered_data_loader(
        val_data, val_traj_loader, args.batch_size, shuffle=False
    )
    if test_data is not None:
        test_loader = create_filtered_data_loader(
            test_data, test_traj_loader, args.batch_size, shuffle=False
        )
    else:
        test_loader = None
    ...
    kinematic_loss_fn = None
    if args.kinematic_weight > 0:
        kinematic_loss_fn = KinematicLoss(
            max_accel=4.0, max_steering_rate=0.52, dt=0.1
        ).to(args.device)
    ...
    if not args.eval_only:
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, indices in enumerate(pbar):
                ...
                # > forward encoder
                if needs_edge_ids:
                    src_embed, _ = encoder.compute_src_dst_node_temporal_embeddings(src, dst, times, edge_ids)
                else:
                    src_embed, _ = encoder.compute_src_dst_node_temporal_embeddings(src, dst, times)
                ...
                # > kinematic loss
                if kinematic_loss_fn is not None:
                    if args.num_modes > 1:
                        best_mode_idx = output['confidences'].argmax(dim=1)
                        pred_for_kin = output['trajectories'][
                            torch.arange(current.size(0), device=args.device),
                            best_mode_idx
                        ]
                    else:
                        pred_for_kin = output['trajectory']
                    loss = loss + args.kinematic_weight * kinematic_loss_fn(pred_for_kin, current)
    ...
    if 'test' in split_loader.splits and test_data is not None:
        ...
        print(f"  Batch size: {args.batch_size} | Runs: 100 | Warmup: 10")

        total_params = count_parameters(encoder, decoder, lane_encoder, map_fusion)

        timing_results = measure_inference_time(
            encoder, decoder, test_data, test_traj_loader, args,
            needs_edge_ids=needs_edge_ids,
            initial_memory_backup=initial_memory_backup
        )

        # > re-reset after benchmark — last benchmark run dirtied the memory
        if needs_edge_ids and initial_memory_backup is not None:
            encoder.memory_bank.reload_memory_bank(initial_memory_backup)

        print(f"\n  {'Metric':<20} {'Value':>12}")
        print(f"  {'-'*34}")
        print(f"  {'Mean latency':<20} {timing_results['mean_ms']:>10.2f}ms")
        print(f"  {'Std':<20} {timing_results['std_ms']:>10.2f}ms")
        print(f"  {'Median':<20} {timing_results['median_ms']:>10.2f}ms")
        print(f"  {'P95':<20} {timing_results['p95_ms']:>10.2f}ms")
        print(f"  {'Per-sample':<20} {timing_results['per_sample_ms']:>10.3f}ms")
        print(f"  {'Throughput':<20} {timing_results['fps']:>10.1f} samples/s")
        print(f"  {'Parameters':<20} {total_params:>12,}")
        ...
        with torch.no_grad():
            for indices in tqdm(test_loader, desc="Test"):
                ...
                for node_ids, node_embeds in [(src, src_embed), (dst, dst_embed)]:
                    
                    batch_node_ids = node_ids  
                    batch_times = times
                    batch_embeds = node_embeds
                    ...
                    batch_embeds = batch_embeds[keep_mask]
                    ...
                    all_node_ids.append(batch_node_ids.copy())
                    all_timestamps.append(batch_times.copy())
        ...
        if len(all_ade_per_mode) > 0:
            ...
            # > save per sample predictions for challenging case analysis
            if args.save_predictions:
                save_data = {
                    ...
                    'gts': gts.cpu(),
                    'states': states.cpu(),
                    'is_target': is_target_all.cpu(),
                    'v2x_ade': v2x_ade.cpu(),
                    'v2x_fde': v2x_fde.cpu(),
                }
                pred_save_path = f"{log_dir}/test_predictions.pt"
                torch.save(save_data, pred_save_path)
                print(f"Saved per sample predictions to {pred_save_path}")
                print(f"  Shape: {preds.shape[0]} samples")
